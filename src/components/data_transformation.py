...
class DataTransformation:
    ...
    def get_data_transformer_object(self, raw_path):
        ...
        try:
            ...
            numerical_columns = [feature for feature in df.columns if df[feature].dtype != 'O' and feature != 'y']
    ...
    def initiate_data_transformation(self, train_path, valid_path, test_path, raw_path):
        ...
        try:
            ...
            preprocessing_obj, encoder_obj = self.get_data_transformer_object(raw_path)

            target_column_name = "y"

            numerical_columns = [feature for feature in df.columns if df[feature].dtype != 'O']

            logging.info("Saperating features and target columns")
            features = df.drop(columns= [target_column_name], axis = 1)
            target = df[target_column_name]

            logging.info("input and target features for training data")

            input_feature_train_df = train_df.drop(columns=[target_column_name], axis = 1)
            target_feature_train_df = train_df[target_column_name]

            logging.info("input and target features for valid data")
            ...
            logging.info("input and target features for test data")

            input_feature_test_df = test_df.drop(columns=[target_column_name], axis = 1)
            target_feature_test_df = test_df[target_column_name]

            logging.info(f"Applying target encoding object on training, valid and testing dataframe")

            X_train_encoded = encoder_obj.fit_transform(input_feature_train_df, target_feature_train_df)
            X_valid_encoded = encoder_obj.transform(input_feature_valid_df, target_feature_valid_df)
            X_test_encoded = encoder_obj.transform(input_feature_test_df, target_feature_test_df)
            X_encoded =  encoder_obj.transform(features, target)
            ...
            df_arr_encoded = np.c_[X_encoded, np.array(target)]
            train_arr_encoded = np.c_[X_train_encoded, np.array(target_feature_train_df)]
            ...
            logging.info("Model Transformation is completed.")
            return(
                df_arr,
                train_arr,
                valid_arr,
